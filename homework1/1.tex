...
\author{
\begin{tabular}{ccc}
\textbf{曹忠昊（组长）} & 侯钧译（组员） & 徐舸山（组员） \\[2pt]
学号 2023211964 & 学号 2023211728 & 学号 2023211038 \\
\end{tabular}\\[0.45cm]
\begin{minipage}{0.86\textwidth}\centering\normalsize
\textbf{分工完成情况：}曹忠昊（组长）负责传感器总体分类梳理与全文统稿；侯钧译负责 CMOS 图像传感器原理与三代架构演进分析；徐舸山负责国内外产业格局对比与可持续发展、展望部分。
\end{minipage}
}
\date{2026 年 3 月}
...
从产业角度看，全球CMOS图像传感器市场在2025年已达到105.99亿美元，预计到2034年将增长至247.38亿美元\cite{statsmarket2026,researchmarkets2025}。中国传感器市场规模已超过4000亿元人民币\cite{eetChina2025}，以豪威集团、思特威、格科微为代表的国产企业正在快速崛起，在中高端市场与索尼、三星展开激烈角逐。与此同时，传感器技术正经历从单一感知向多模态融合、从硬件竞赛向软硬协同、从消费电子向汽车和医疗等领域渗透的深刻变革。
...
为确保本调研报告的全面性、客观性和学术严谨性，笔者在报告撰写过程中遵循了以下调研方法论：

\subsection{文献检索策略}

本调研采用多源交叉验证的文献检索策略，主要检索渠道包括：
\begin{enumerate}[label=(\arabic*)]
    \item \textbf{学术数据库：}通过 IEEE Xplore、Web of Science、CNKI（中国知网）和 Google Scholar 检索"CMOS image sensor"、"smartphone sensor"、"computational photography"等关键词，筛选高被引和近五年内的期刊与会议论文；
    \item \textbf{行业研究报告：}参考 Yole Développement、IC Insights、Stats Market Research 等专业半导体市场研究机构发布的 CMOS 图像传感器市场报告；
    \item \textbf{企业公开信息：}查阅索尼、三星、豪威集团、思特威、格科微等主要厂商的财报、产品白皮书和技术发布会资料；
    \item \textbf{政策与标准文件：}检索中国政府网、国家发改委、工信部等部门发布的传感器产业政策和环保法规。
\end{enumerate}

\subsection{资料来源筛选标准}

在数百篇初步检索结果中，本报告依据以下标准筛选核心参考资料：
\begin{enumerate}[label=(\arabic*)]
    \item \textbf{权威性：}优先采用同行评审的学术论文（如 IEEE Transactions on Electron Devices、Sensors 期刊）、头部企业官方数据和政府文件；
    \item \textbf{时效性：}以 2022--2026 年间的文献为主，技术演进和产业现状部分使用最新资料；
    \item \textbf{可验证性：}关键数据（如市场份额、技术参数）需至少有两个独立来源交叉印证。
\end{enumerate}

\subsection{分析的局限性}

本调研存在以下局限：（1）部分前沿技术（如量子传感器、脑机接口）的学术文献较少，相关论述主要基于技术预研报告和企业研发动态，结论具有一定推测性；（2）市场份额数据因统计口径差异，不同来源间存在约 2\%--5\% 的偏差，本报告取多源平均值或注明数据来源；（3）因调研周期限制，部分 2026 年下半年最新动态未能充分纳入。
...
\textbf{（1）图像与光学感知类传感器。}包括CMOS图像传感器（主摄像头、超广角、长焦、前置摄像头）、飞行时间（Time-of-Flight, ToF）深度传感器、结构光传感器（如iPhone的Face ID）和激光雷达（LiDAR）扫描仪。这类传感器赋予手机"看"的能力，是手机摄影、人脸识别、增强现实（AR）等功能的硬件基础\cite{elgamal2005}。
...
\textbf{（4）触摸与力感知类传感器。}包括电容式触摸屏传感器、压力传感器（如Apple的3D Touch/Haptic Touch）和超声波指纹传感器。电容式触摸屏通过检测人体手指接近时引起的电容变化来定位触摸位置，支持多点触控；超声波指纹传感器利用超声波穿透OLED屏幕采集指纹3D信息，实现屏下指纹解锁\cite{adenekan2024}。
...
\textbf{（6）生物特征与健康感知类传感器。}包括光电容积脉搏波（PPG）传感器（用于心率和血氧测量）、红外温度传感器和光谱传感器\cite{allen2007}。然而，目前仅32\%的Android设备和68\%的iPhone在心率和血氧测量方面达到临床级精度标准\cite{alibaba2026}，该领域仍有较大提升空间。
...
CMOS图像传感器的核心工作原理基于半导体的\textbf{内光电效应}\cite{fossum1997,baidu_cmos}：当入射光子的能量大于硅材料的禁带宽度（约1.12 eV，对应波长$\sim$1100 nm）时，光子被硅吸收并激发出电子-空穴对，从而将光信号转换为电信号。
...
六轴IMU（加速度计+陀螺仪）是智能手机中数据输出最为可靠的传感器之一\cite{alibaba2026}。其应用场景已从早期的屏幕旋转和游戏交互，扩展至惯性导航、运动健康分析、地震预警等专业级场景。MEMS陀螺仪的零偏稳定性从早期的$>10^{\circ}$/h 提升至当前的 $<1^{\circ}$/h，结合磁力计和气压计的多传感器融合定位精度已能达到室内亚米级\cite{mdpi2024}。
...
多项前沿技术正在加速从实验室走向商业化\cite{innosensor2025,gansuan2025}：（1）\textbf{量子传感器}利用量子态的极端敏感性实现超高精度测量，可将磁场检测灵敏度提升数个数量级；（2）\textbf{光子集成电路（PIC）传感器}将光学器件集成在芯片上，实现片上光谱分析，有望让智能手机具备物质成分检测能力；（3）\textbf{太赫兹传感器}随6G通信技术的发展，"通信+传感"一体化将使手机兼具精密成像和安全检测功能；（4）\textbf{QD-on-CMOS短波红外（SWIR）传感器}通过量子点技术降低SWIR传感器成本，扩展手机在农业、食品安全检测等领域的应用。
...
\textbf{（1）电子废弃物问题。}智能手机中包含超过60种元素材料\cite{stcn2025}，其中多种为稀有金属和重金属。一块手机电池中的镉可污染6万升水\cite{ndrc2022}。中国每年产生4亿部以上废旧手机，但资源回收利用率仅约4\%，约54.2\%被消费者闲置\cite{stcn2025}。此外，废弃智能手机在水生系统中的元素浸出行为具有一致性特征，构成潜在环境风险\cite{ebiotrade2025}。
...
未来的智能手机将不再依赖单一传感器完成特定任务，而是通过\textbf{多模态传感融合}实现更高层次的环境感知\cite{hall1997}。全球传感器融合市场预计到2032年将达到319.1亿美元，年复合增长率18.65\%\cite{innosensor2025}。例如，将图像传感器的视觉信息、IMU的运动数据、麦克风的声学信号和气压计的环境数据融合在一起，手机可以构建出对周围世界的多维度理解，从"感知单一物理量"跃升至"理解复杂场景"。
...
本报告以"智能手机传感器的国内外发展情况"为题，采用多源交叉验证的调研方法，在全面梳理六大类传感器功能体系的基础上，以CMOS图像传感器为重点，完成了从物理原理、技术演进到产业格局的全链条分析。综合本报告各部分的调研与分析，可以凝练出以下四个核心认识：

\textbf{第一，传感器技术正经历从"单一器件优化"到"系统级协同创新"的范式转变。}三代CMOS架构（FSI$\to$BSI$\to$Stacked）的演进表明，单纯增大像素面积或提高像素数已触及物理极限，真正的突破来自架构创新（如堆叠式芯片的像素层与电路层分离制造）和软硬协同（如计算摄影中的多帧融合与AI超分辨率）。这一规律对理解其他传感器（如MEMS麦克风、IMU）的技术路线同样具有启示意义。

\textbf{第二，国内外CIS产业的差距正在从"量变"走向"质变"。}豪威集团、思特威、格科微三家国产企业不仅在中低端市场以出货量确立优势（格科微全球出货量第二），更在5000万像素高端产品线上形成对索尼、三星的实质性竞争。但需清醒认识到，国产企业在核心IP（如双层晶体管像素）、先进制程（22 nm以下）和自有晶圆产能方面仍有显著差距，真正的"技术主权"尚未建立。

\textbf{第三，传感器的社会价值远超消费电子范畴，但可持续性挑战不容回避。}手机传感器在全民健康监测（SDG 3）、环境与灾害预警（SDG 13）、智慧农业（SDG 2）和信息无障碍（SDG 10）方面展现出巨大的正向外部性。然而，年产4亿部以上废旧手机仅4\%的回收利用率，以及传感器制造对稀土、关键矿物和大量水资源的依赖，表明产业界距离"绿色传感"目标仍有漫长道路。

\textbf{第四，未来十年是智能手机传感器从"感知"走向"认知"的关键窗口期。}AI原生传感器设计、多模态传感融合、事件驱动感知等前沿方向的突破，将使手机从被动的物理量采集器转变为主动的环境理解者。与此同时，电子鼻/舌、脑机接口、太赫兹等新型传感模态的引入，有望在五感覆盖、非接触交互和安全检测等领域开启全新的应用范式。

综合而言，智能手机传感器技术正处于一个"黄金交叉点"：硬件创新远未见顶，AI与传感器的深度融合才刚刚开始，而可持续发展理念正在重新定义传感器从设计、制造到回收的全生命周期。国产传感器产业能否在技术深度、生态广度和可持续性三个维度上同步突破，将决定中国在全球传感产业格局中的长期地位。
...
\bibitem{fossum1997}
    ...
    \textit{IEEE Transactions on Electron Devices}, 1997, 44(10): 1689--1698.

\bibitem{elgamal2005}
    El Gamal A, Eltoukhy H.
    CMOS Image Sensors.
    \textit{IEEE Circuits and Devices Magazine}, 2005, 21(3): 6--20.

\bibitem{allen2007}
    Allen J.
    Photoplethysmography and Its Application in Clinical Physiological Measurement.
    \textit{Physiological Measurement}, 2007, 28(3): R1--R39.

\bibitem{hall1997}
    Hall D L, Llinas J.
    An Introduction to Multisensor Data Fusion.
    \textit{Proceedings of the IEEE}, 1997, 85(1): 6--23.

\bibitem{yole2025}
    Yole Développement.
    Status of the CMOS Image Sensor Industry 2025.
    Yole Intelligence, 2025.
