...
def evaluate_extraction_human(predictions, judgments):
    """人工逐字段评价 (用于"人工准确率评价"档位)

    judgments: {field: 'correct'|'partial'|'wrong'}  —— 评价者对每个抽取字段的判定
    评分: correct=1.0, partial=0.5, wrong=0.0; 准确率 = 已判定字段得分均值。
    """
    score_map = {'correct': 1.0, 'partial': 0.5, 'wrong': 0.0}
    per_field = {}
    scores = []
    for field, verdict in judgments.items():
        s = score_map.get(verdict, 0.0)
        pred = predictions.get(field, '')
        pred_str = ' '.join(pred) if isinstance(pred, list) else str(pred)
        per_field[field] = {'verdict': verdict, 'score': s, 'value': pred_str[:120]}
        scores.append(s)
    n = len(scores)
    return {
        'per_field': per_field,
...
class EvaluationStore:
    ...
    def save_extraction_eval(self, doc_id, predictions, detail, metrics):
        record = {
            'type': 'extraction',
            'doc_id': doc_id,
            'predictions': predictions,
            'detail': detail,          # 人工评价时为 judgments; 自动评价时为 ground_truth
            'metrics': metrics,
            'timestamp': datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
        }
    ...
    def get_extraction_summary(self):
        ...
        # 兼容两类记录: 人工逐字段(accuracy) / 自动pred-vs-gt(exact_accuracy)
        accuracies = []
        for r in self.extraction_records:
            m = r.get('metrics', {})
            if 'accuracy' in m:
                accuracies.append(m['accuracy'])
            elif 'exact_accuracy' in m:
                accuracies.append(m['exact_accuracy'])
        return {
