"""
VLM 图片质检过滤器 — 用视觉大模型 (Qwen-VL) 剔除灾害新闻中的无关配图

灾害新闻页抓取的配图里混有大量与灾情无关的噪声: 站标 logo、UI 图标 / 按钮、
广告横幅、二维码、纯文字快讯图 / 海报、网页 / App 界面截图、纯装饰图、用户头像等。
仅靠"来源 URL 关键词 + 最小边像素"的廉价规则无法识别这些"看起来是图、实为噪声"的内容,
它们会污染 OCR 抽取与配图展示 (典型如澎湃、地震局等站点的 logo 被当成配图)。

本模块对每张图 (降采样到 512px) 做一次语义级质检, 判断它是否为
"有意义的灾害新闻内容图", 只保留通过者用于 OCR / 抽取 / 展示。

设计要点 (与作业2 的图片质检一致):
- **缓存**: 判定结果按 文件名 -> verdict 持久化到 data/vectors/vlm_verdicts.json,
  重复运行零 API 调用; 缓存随交付分发, 即使无 API Key 也能复现"已清洗"的结果。
...
# 质检提示词: 与"灾害新闻配图"目标对齐 —— 保留真实灾情图像, 剔除各类噪声 / UI / 广告。
_PROMPT = (
    "你是灾害新闻图片库的质检员。判断这张图片是否为『有意义的灾害新闻内容图』——"
    "即与灾害事件相关的真实图像: 灾害现场、受灾场景、抢险救援、相关人物、受损实物、"
    "灾情统计图表、震中 / 受灾示意图、卫星云图、气象预警图等, 适合作为灾害新闻配图。"
    "若属于以下任一类无关 / 噪声, 判为不保留: 网站 logo / 品牌标识、图标 / 按钮 / 导航等 UI 元素、"
    "广告或促销横幅、二维码、与灾害无关的纯文字图 / 海报、网页或 App 界面截图、"
    "纯装饰图 (纯色 / 渐变 / 花纹)、与灾害新闻无关的通用素材图、水印占主导、表情包、用户头像。"
...
class VLMFilter:
    """用视觉大模型对灾害配图做"内容图 vs 噪声"二分类, 带磁盘缓存与并发。"""

    def __init__(self):
        cfg = MULTIMODAL_CONFIG
        self.model = cfg.get('vlm_model', 'qwen3-vl-plus')
    ...
    def verdict(self, basename):
        """查单图裁决: True=保留 / False=剔除 / None=无裁决 (按 fail-open 当作保留)。"""
        v = self.cache.get(basename)
        if v is None:
            return None
        return bool(v.get('keep'))
