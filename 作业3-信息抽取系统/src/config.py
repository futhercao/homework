...
CRAWLER_CONFIG = {
    ...
    # 通用新闻种子URL (14源/5类) — 仅供共享通用爬虫复用。
    # 注: 作业3 的灾害定向采集走 scripts/crawl_disaster.py 内的 DISASTER_QUERIES + SEED_URLS
    #     (政府/应急/气象/通讯社等灾害源 + 灾害关键词驱动的链接发现), 不使用下面的通用种子。
    'seed_urls': [
        # 科技类
...
MULTIMODAL_CONFIG = {
    ...
    # 多语言 CLIP: 支持中文"以文搜图"查询 (xlm-roberta 文本塔 + ViT-B-32 视觉塔)
    ...
    # VLM 图片质检: 用视觉大模型 (Qwen-VL) 剔除 logo / UI / 广告 / 二维码 / 纯文字海报 / 截图
    # 等"无关配图", 仅保留真实灾害新闻内容图。判定结果缓存到 data/vectors/vlm_verdicts.json
    # (随交付分发, 无密钥也能复现"已清洗"的结果); 密钥仅从环境变量读取, 绝不写入文件。
    'vlm_enabled': True,
